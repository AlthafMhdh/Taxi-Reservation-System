...
const Booking = () => {
  ...
  const validateform2 = () => {
    const { Pickup, Dropping, name, number } = state;
    const errors = {};
    ...
    if (!number) {
      errors.number = 'Passenger Phone Number is Required.'
    }
      ...
      directionsService.route(
        ...
        (response, status) => {
          if (status === 'OK') {
           ...
           // setDistance(leg.distance.text);
           // setDuration(leg.duration.text);
          //  setTDistance(leg.Tdistance.text);
  ...
  const submitform = () => {
    ...
    if (isValid) {
      ...
      axios
        .post('http://localhost:8082/api/v1/booking/save', bookingData)
        .then((response) => {
